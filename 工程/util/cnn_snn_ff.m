    ...
    for l = 2 : n-1   %  for each layer
        if strcmp(net.layers{l}.type, 'c')
            ...
            for j = 1 : net.layers{l}.outputmaps   %  for each output map
                ...
                %  ����BN����
                %在这里用泄漏L来代替测试的BN操作，进行spike判断
                net.layers{l}.a_hat{j} = net.layers{l}.a_pre{j} + net.Leakage{l}{j};
                
                %potential{l}{j}=net.layers{l}.a_hat{j};
...
%                    if l==4
%                       if j==3
%                          %gamma尽量要符号一致，�?��>0,便于适应转换到snn的大于阈值spike的情�?%                          
%                          net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} < 0)) = 1;
%                          net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} >= 0)) = 0;
%                       else
%                            net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} > 0)) = 1;
%                            net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} <= 0)) = 0;
%                       end
%                    else
                      net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} >=1)) = 1;          
                      net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} <= 0)) = 0;
%                    end
                   net.layers{l}.a{j} = net.layers{l}.a_hat{j};
...
%                    end
                   ...
                   if strcmp(net.layers{l}.activation, 'sign')
                      net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} >= 1)) = 1;
                      net.layers{l}.a_hat{j}(find(net.layers{l}.a_hat{j} <= 0)) = 0;
                      net.layers{l}.p{j} = net.layers{l}.a_hat{j};                    
